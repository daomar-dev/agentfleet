...
const pages = ['about.html', 'donate.html'];

for (const page of pages) {
  const html = readFileSync(resolve(publicDir, page), 'utf8');
...
const script = readFileSync(resolve(publicDir, 'static-i18n.js'), 'utf8');
...
for (const page of ['', 'about.html', 'donate.html']) {
  ...
  assert(sitemap.includes(`https://agentfleet.daomar.dev${path}?lang=en-US`), `sitemap missing English alternate for ${path}`);
  assert(sitemap.includes(`https://agentfleet.daomar.dev${path}?lang=zh-CN`), `sitemap missing Chinese alternate for ${path}`);
}
