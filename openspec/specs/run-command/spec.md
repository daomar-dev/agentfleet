## Purpose
Define the run command that serves as the single primary command for operating Lattix on a machine.

## Requirements

### Requirement: Run command starts the task watcher
The system SHALL provide a `run` command that starts the task watcher in the foreground. This is the single primary command for operating Lattix on a machine.

#### Scenario: Starting Lattix with existing configuration
- **WHEN** the user runs `lattix run` and a valid `~/.lattix/config.json` exists
- **THEN** the system SHALL load the existing configuration, validate symlinks, and start the task watcher

#### Scenario: Starting Lattix without prior configuration
- **WHEN** the user runs `lattix run` and no `~/.lattix/config.json` exists
