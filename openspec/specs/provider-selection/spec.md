...
The system SHALL persist the provider metadata required to reproduce the chosen sync target in the local AgentFleet configuration. The provider is always `onedrive`.

#### Scenario: First-time initialization
- **WHEN** bootstrap completes successfully for the first time
- **THEN** the system SHALL write the provider (`onedrive`) and the detected OneDrive sync target metadata to `~/.agentfleet/config.json`

#### Scenario: Subsequent runs with existing configuration
- **WHEN** bootstrap runs and a valid `~/.agentfleet/config.json` already exists
