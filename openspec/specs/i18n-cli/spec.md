...
1. The `AGENTFLEET_LANG` environment variable (if set)
...
- **WHEN** the OS display language is `zh-CN` or `zh-Hans` and `AGENTFLEET_LANG` is not set
...
#### Scenario: AGENTFLEET_LANG override
- **WHEN** the `AGENTFLEET_LANG` environment variable is set to `zh-CN`
...
- **WHEN** the detected locale is `en-US` and the user runs `agentfleet --help`
...
- **WHEN** the locale is `zh-CN` and the user runs `agentfleet run`
...
The CLI's `--json` output flag SHALL produce locale-independent output. JSON-formatted output from commands like `agentfleet status --json` SHALL remain in English regardless of the active locale. Only human-readable text output SHALL be affected by locale settings.

#### Scenario: JSON output unaffected by zh-CN locale
- **WHEN** the locale is `zh-CN` and the user runs `agentfleet status --json`
- **THEN** the JSON output SHALL contain English field names and values, identical to en-US locale output

#### Scenario: AGENTFLEET_LANG override for scripts
- **WHEN** a script sets `AGENTFLEET_LANG=en-US` before invoking a CLI command
...
- **WHEN** the locale is `zh-CN` and the user runs `agentfleet status`
- **THEN** date and time values in the output SHALL use Chinese date formatting conventions

#### Scenario: Status dates in English
- **WHEN** the locale is `en-US` and the user runs `agentfleet status`
