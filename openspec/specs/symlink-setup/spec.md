## Purpose
Define the local AgentFleet home layout, OneDrive symlink targets, permission fallback behavior, and legacy migration rules.

## Requirements

### Requirement: Migrate legacy agentbroker storage paths
The system SHALL detect an existing legacy `agentbroker` setup and SHALL migrate or reuse it when adopting the AgentFleet path layout, provided no conflicting AgentFleet paths already exist.

#### Scenario: Safe migration from legacy paths
- **WHEN** `~/.agentbroker` exists and `~/.agentfleet` does not exist
- **THEN** the system SHALL migrate or reuse the legacy local directory contents under `~/.agentfleet` before continuing setup
...
### Requirement: Create AgentFleet home directory
The system SHALL create a `~/.agentfleet` directory in the user's home directory if it does not already exist. This directory serves as the stable entry point for all AgentFleet file operations.

#### Scenario: First-time setup
- **WHEN** the system starts for the first time and `~/.agentfleet` does not exist
- **THEN** the system SHALL create `~/.agentfleet` directory and proceed with symlink creation

#### Scenario: Directory already exists
- **WHEN** the system starts and `~/.agentfleet` already exists with valid symlinks
...
The system SHALL create symbolic links inside `~/.agentfleet` that point to corresponding directories within the selected OneDrive sync folder. The symlink targets SHALL be:
- `~/.agentfleet/tasks` → `<OneDrivePath>\AgentFleet\tasks`
- `~/.agentfleet/output` → `<OneDrivePath>\AgentFleet\output`
...
- **THEN** the system SHALL create the `AgentFleet\tasks` and `AgentFleet\output` directories in the selected OneDrive folder if they do not exist, and create symlinks from `~/.agentfleet/tasks` and `~/.agentfleet/output` pointing to those directories
...
The system SHALL create a `~/.agentfleet/config.json` file to store local machine configuration including the selected provider, the selected OneDrive path, and machine-specific settings.
