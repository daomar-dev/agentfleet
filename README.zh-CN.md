<p align="center">
  <img src="assets/icon.svg" alt="Lattix 图标" width="160" />
</p>
...
Lattix 让您可以将编码任务同时分发到所有机器上。从任意机器提交任务，所有运行 Lattix 的机器都会看到同一个任务，使用本地安装的编码智能体独立执行，并将机器级别的结果写回。

**工作原理：**
1. 您的机器共享一个同步的 OneDrive 工作区。
2. Lattix 在 `~/.lattix/` 下提供稳定的本地路径。
3. 任何机器都可以在共享的 `tasks/` 目录中创建任务。
4. 每台运行 `lattix run` 的机器都会独立处理该任务。
...
首次运行时，Lattix 会自动在 npm 全局目录中创建 `lattix` 快捷命令，因此您可以直接使用 `lattix`——无需重启终端即刻生效。如果您已通过 `npm install -g lattix` 全局安装，将跳过快捷方式创建。
...
在当前机器上启动 Lattix。首次使用时，它会自动检测 OneDrive 并创建必要的符号链接和配置。后续运行时，它会加载现有配置并启动任务监视器。只有在 Lattix 启动**之后**到达的任务才会被处理——已有的任务永远不会被重放，即使在重启或新机器上也是如此。
...
- `--log-file <path>` — 作为守护进程运行时的日志文件路径（默认：`~/.lattix/lattix.log`）

#### 守护进程模式

在后台运行 Lattix，使其在关闭终端后继续运行：

```bash
lattix run --daemon
```

这将生成一个分离的进程，将其 PID 写入 `~/.lattix/lattix.pid`，并将所有输出重定向到 `~/.lattix/lattix.log`。同一时间只允许一个守护进程实例。
...
> **注意：** 同一时间只允许一个 Lattix 实例运行，无论是前台模式、守护进程模式还是自动启动模式。

#### 登录时自动启动

对于需要 Lattix 常驻运行的机器，设置登录时自动启动：

```bash
npx -y lattix install
```

这将创建一个计划任务，在每次登录以及计算机从睡眠或休眠唤醒时运行 `npx lattix run -d`（始终使用最新版本）。安装后守护进程会立即启动。无需管理员权限。
...
这会向运行中的进程发送 SIGTERM 信号并清理 PID 文件。适用于所有运行模式（前台、守护进程、自动启动）。如果配置了自动启动，Lattix 将在下次登录或从睡眠唤醒时重新启动。

Lattix 会检查 OneDrive 商业版和个人版账户。

- 如果恰好有一个支持的账户可用，Lattix 会自动使用它。
- 如果个人版和商业版 OneDrive 同时可用，Lattix 会选择第一个检测到的。

这将创建 `~/.lattix/` 并将其 `tasks/` 和 `output/` 目录指向您选定的 OneDrive 工作区中的 `Lattix/`。
...
显示版本信息、运行中的 Lattix 进程信息（PID、模式、日志文件），并列出所有任务及其各机器的结果：
...
├── lattix.pid           # PID 文件（任何模式运行时存在）
├── lattix.log           # 日志文件（守护进程和自动启动模式）
├── start-lattix.vbs     # 自动启动启动器（由 install 创建）
├── tasks/ → OneDrive    # 指向选定的 <OneDrive>\Lattix\tasks 的符号链接
│   ├── task-001.json
│   └── task-002.json
└── output/ → OneDrive   # 指向选定的 <OneDrive>\Lattix\output 的符号链接
    ├── task-001/
...
Lattix 采用零基础设施安全模型设计：

- **无隧道** — 机器永远不会打开入站连接或隧道。外部没有任何可攻击的入口。
- **无暴露端口** — 没有监听服务、没有开放端口、没有攻击面。每台机器仅通过 OneDrive 现有的、已认证的通道同步文件。
- **无数据移动** — Lattix 不传输、复制或中转任何数据。任务文件和结果存储在用户自己的 OneDrive（商业版或个人版）中。数据永远不会离开 Microsoft 365 租户边界。
- **无中央服务器** — 没有 Lattix 后端、代理或控制平面。协调完全通过 OneDrive 同步完成，该同步已获得您组织 IT 策略的批准和管理。

这种架构意味着 Lattix 继承了您现有 OneDrive 和 Microsoft 365 环境的安全性、合规性和数据驻留保证——无需额外审计、安全加固或维护。
...
测试套件涵盖任务监视器启动行为、快捷方式注册、计划任务触发器、守护进程管理、run/install/stop/uninstall 命令、CLI 品牌标识、引导程序、OneDrive 检测、提供者选择、结果写入和旧工作区迁移。

## Web 仪表板

基于浏览器的仪表板可在 **[https://lattix.code365.xyz/](https://lattix.code365.xyz/)** 访问。

### 功能

- **提交任务** — 从任何浏览器同时向所有 Lattix 机器提交任务
...
在浏览器中打开 **[https://lattix.code365.xyz/](https://lattix.code365.xyz/)**，使用拥有 Lattix 所在 OneDrive 的 Microsoft 账户登录。
...
- **无后端：** 所有 API 调用直接从浏览器发送到 Microsoft Graph——不涉及 Lattix 服务器

### 移动端安装（PWA）

**iOS (Safari)：** 打开 [https://lattix.code365.xyz/](https://lattix.code365.xyz/) → 点击**分享**按钮 → **添加到主屏幕**

**Android (Chrome)：** 打开 [https://lattix.code365.xyz/](https://lattix.code365.xyz/) → 点击 **⋮** 菜单 → **添加到主屏幕**（或等待安装提示）
...
| 重定向 URI | `https://lattix.code365.xyz/`、`http://localhost:5173/` |
